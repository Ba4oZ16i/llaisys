...
Qwen2Model::Qwen2Model(LlaisysQwen2Meta meta, llaisysDeviceType_t device, int *device_id, int ndevice)
    : meta_(meta), device_(device), device_id_(device_id), ndevice_(ndevice) {
...
int64_t Qwen2Model::infer(int64_t *token_ids, size_t ntoken) {
    ...
    tensor_t new_token_ids = Tensor::create({new_token_num}, LLAISYS_DTYPE_I64, device_, device_id_[0]);
    std::memcpy(new_token_ids->data(), token_ids + cache_len_, new_token_num * sizeof(int64_t));
    tensor_t new_token_input = Tensor::create({new_token_num, meta_.hs}, meta_.dtype, device_, device_id_[0]);
    llaisys::ops::embedding(new_token_input, new_token_ids, in_embed_.tensor);

    for (size_t i = 0; i < meta_.nlayer; i++) {
        tensor_t normed = Tensor::create({new_token_num, meta_.hs}, meta_.dtype, device_, device_id_[0]);
        llaisys::ops::rms_norm(normed, new_token_input, attn_norm_w_[i]->tensor, meta_.epsilon);

        tensor_t q_temp = Tensor::create({new_token_num, meta_.nh * meta_.dh}, meta_.dtype, device_, device_id_[0]);
        ...
        tensor_t k_temp = Tensor::create({new_token_num, meta_.nkvh * meta_.dh}, meta_.dtype, device_, device_id_[0]);
        ...
        tensor_t v_temp = Tensor::create({new_token_num, meta_.nkvh * meta_.dh}, meta_.dtype, device_, device_id_[0]);
        ...
        tensor_t q_rope = Tensor::create({new_token_num, meta_.nh, meta_.dh}, meta_.dtype, device_, device_id_[0]);
        tensor_t k_rope = Tensor::create({new_token_num, meta_.nkvh, meta_.dh}, meta_.dtype, device_, device_id_[0]);
        tensor_t pos_ids = Tensor::create({new_token_num}, LLAISYS_DTYPE_I64, device_, device_id_[0]);
        ...
        tensor_t attn_out = Tensor::create({new_token_num, meta_.nh, meta_.dh}, meta_.dtype, device_, device_id_[0]);
        ...
        tensor_t attn_out_fin = Tensor::create({new_token_num, meta_.hs}, meta_.dtype, device_, device_id_[0]);
        llaisys::ops::linear(attn_out_fin, attn_out, attn_o_w_[i]->tensor, nullptr);

        tensor_t new_token_input1 = Tensor::create({new_token_num, meta_.hs}, meta_.dtype, device_, device_id_[0]);
        llaisys::ops::add(new_token_input1, new_token_input, attn_out_fin);

        tensor_t normed1 = Tensor::create({new_token_num, meta_.hs}, meta_.dtype, device_, device_id_[0]);
        llaisys::ops::rms_norm(normed1, new_token_input1, mlp_norm_w_[i]->tensor, meta_.epsilon);

        tensor_t gate = Tensor::create({new_token_num, meta_.di}, meta_.dtype, device_, device_id_[0]);
        llaisys::ops::linear(gate, normed1, mlp_gate_w_[i]->tensor, nullptr);
        tensor_t swiglu_out = Tensor::create({new_token_num, meta_.di}, meta_.dtype, device_, device_id_[0]);
        tensor_t up = Tensor::create({new_token_num, meta_.di}, meta_.dtype, device_, device_id_[0]);
        llaisys::ops::linear(up, normed1, mlp_up_w_[i]->tensor, nullptr);
        llaisys::ops::swiglu(swiglu_out, gate, up);

        tensor_t mlp_out = Tensor::create({new_token_num, meta_.hs}, meta_.dtype, device_, device_id_[0]);
        llaisys::ops::linear(mlp_out, swiglu_out, mlp_down_w_[i]->tensor, nullptr);

        tensor_t new_token_input2 = Tensor::create({new_token_num, meta_.hs}, meta_.dtype, device_, device_id_[0]);
        llaisys::ops::add(new_token_input2, new_token_input1, mlp_out);
        new_token_input = new_token_input2;
    }
    tensor_t fin_norm = Tensor::create({new_token_num, meta_.hs}, meta_.dtype, device_, device_id_[0]);
    llaisys::ops::rms_norm(fin_norm, new_token_input, out_norm_w_.tensor, meta_.epsilon);
    tensor_t re_voc = Tensor::create({new_token_num, meta_.voc}, meta_.dtype, device_, device_id_[0]);
    ...
    tensor_t max_idx = Tensor::create({1}, LLAISYS_DTYPE_I64, device_, device_id_[0]);
    tensor_t max_val = Tensor::create({1}, meta_.dtype, device_, device_id_[0]);
