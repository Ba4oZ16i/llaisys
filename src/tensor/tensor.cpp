...
bool Tensor::isContiguous() const {
  size_t stride = 1;
  for (size_t i = this->shape().size(); i > 0; i--) {
    if (this->strides()[i - 1] != (ptrdiff_t)stride)
      return false;
...
tensor_t Tensor::permute(const std::vector<size_t> &order) const {
  ...
  std::vector<ptrdiff_t> new_strides(size);
  std::vector<size_t> new_shape(size);
...
tensor_t Tensor::slice(size_t dim, size_t start, size_t end) const {
  ...
  std::vector<size_t> new_shape(size);
  for (size_t i = 0; i < size; i++) {
    i == dim ? new_shape[i] = end - start : new_shape[i] = this->shape()[i];
  }
  TensorMeta new_meta{this->dtype(), new_shape, this->strides()};
  size_t new_off =
      this->_offset + start * this->strides()[dim] * this->elementSize();
  return std::shared_ptr<Tensor>(new Tensor(new_meta, _storage, new_off));
}

void Tensor::load(const void *src_) {
  core::context().setDevice(this->deviceType(), this->deviceId());
  auto *api = core::context().runtime().api();
  api->memcpy_sync(this->data(), src_, this->elementSize() * this->numel(),
                   LLAISYS_MEMCPY_H2D);
}
